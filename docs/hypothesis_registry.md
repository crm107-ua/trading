...
**Estado del proyecto (2026-07-15):** Binance **CERRADO CON RESPUESTA** (#1–#14). Polymarket **VIVA (research)** — #15 `NO_EVALUABLE`; **#16 pre-reg congelado** (maker post-only); **#17 eval LLM naive ABORTADO** (cierre estratégico, sin veredicto empírico).
...
| 16 | **Polymarket maker stale-quote:** publicar bid/ask anclados a fair value; cancel/replace cuando spot se mueve ≥25 USD — **post-only GTC** | BTC Up/Down 5m; token Up solo | Fase A: grabación ≥30d WS · Fase B: paper maker ≥14d · Fase C: screen único `sim_maker_quote.py` | Pre-reg `polymarket/docs/PREREG_16_POLY_MAKER_STALE.md`; half_spread=1.5¢, σ=0.55, adverse>55%=muerte | **NO_INICIADO** — fase A nunca desplegada; pre-reg + tooling conservados | 2026-07-13 |
| 17 | **LLM naive forecast vs mercado:** `meta/llama-3.3-70b-instruct` (NVIDIA NIM) predice probabilidad vs mid Polymarket — **sin retrieval**, pipeline `naive` | Binarios Polymarket filtrados (≥7d, líquidos); muestra 500 / 1.471 snapshots | Eval v1 `llm-forecast-lab`: ingest→gate→forecast→score→report; métrica Brier/skill held-out (~150 preguntas) | Pre-reg `config/eval_frozen.json` + `docs/PROTOCOL.md`; veredicto vinculante solo si run completo; **no** claim PnL | **ABORTADO** — forecast 375/1471 (~25,5%), 0 fallos; cierre estratégico 2026-07-15 antes de veredicto empírico — sin vía creíble a dinero; no reabrir naive ni variantes sin pre-reg nuevo | 2026-07-15 |
